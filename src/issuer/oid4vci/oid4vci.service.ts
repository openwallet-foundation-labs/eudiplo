...
export class Oid4vciService {
  ...
  createOffer(body: OfferRequest): Promise<OfferResponse> {
    body.credentialConfigurationIds.forEach((id) => {
      if (
        ...
        throw new ConflictException('Invalid credential configuration ID');
      }
    });
  ...
  async getCredential(req: Request): Promise<CredentialResponse> {
    ...
    const { tokenPayload } = await this.resourceServer.verifyResourceRequest({
      authorizationServers: this.issuerMetadata().authorizationServers,
      request: {
        url: `https://${req.host}${req.url}`,
        method: req.method as HttpMethod,
        headers,
      },
      resourceServer: this.issuerMetadata().credentialIssuer.credential_issuer,
      allowedAuthenticationSchemes: [SupportedAuthenticationScheme.DPoP],
    });

    const session = await this.sessionService.get(tokenPayload.sub as string);

    const credentials: string[] = [];
    for (const jwt of parsedCredentialRequest.proofs.jwt) {
      const verifiedProof = await this.issuer.verifyCredentialRequestJwtProof({
