...
export class CredentialsService {
  ...
  async getCredentialConfigurationSupported(
    ...
    for (const value of configs) {
      ...
      value.config.vct = `${this.configService.getOrThrow<string>(
        'PUBLIC_URL'
      )}/${session.tenantId}/credentials/vct/${value.id}`;
  ...
  async getCredential(
    ...
    const binding = issuanceConfig.credentialIssuanceBindings.find(
      (binding) => binding.credentialConfigId === credentialConfigurationId
    );

    const keyId =
      binding?.credentialConfig?.keyId ??
      (await this.cryptoService.keyService.getKid(session.tenantId, 'signing'));
    ...
    const credentialConfig = await this.credentialConfigRepo.findOneByOrFail({
      id: credentialConfigurationId,
      tenantId: session.tenantId,
    });
    ...
    return sdjwt.issue(
      {
        ...
        iat,
        exp,
        vct: `${this.configService.getOrThrow<string>('PUBLIC_URL')}/${
          session.tenantId
        }/credentials/vct/${credentialConfigurationId}`,
        cnf,
        ...claims,
        ...status,
      },
