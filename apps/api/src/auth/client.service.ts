...
export class ClientService implements OnApplicationBootstrap {
    ...
    return [
      {
        ...
        secret: this.configService.getOrThrow<string>('AUTH_CLIENT_SECRET'),
      },
    ];
  }
  ...
  async isSetUp(id: string) {
    void this.clientRepository.countBy({ status: 'set up' }).then((count) => {
      this.tenantClientTotal.set({}, count);
    });
    ...
    await this.clientRepository.findOneByOrFail({ id }).then(
      ...
      async () => {
        ...
        return this.clientRepository.update({ id }, { status: 'set up' });
      }
    );
  }
  ...
  async setUpClient(id: string) {
    const folder = join(this.configService.getOrThrow<string>('FOLDER'), id);
