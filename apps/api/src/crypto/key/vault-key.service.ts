...
export class VaultKeyService extends KeyService {
    ...
    return firstValueFrom(
      ...
      return Promise.all(
        res.data.data.keys.map(async (id: string) => {
          const publicKey = await this.getPublicKey('jwk', tenantId, id);
  ...
  async getPublicKey(
    ...
    return firstValueFrom(
      ...
      return type === 'pem'
        ...
        : await this.getJWK(res.data.data.keys['1'].public_key, tenantId);
    });
  }
   ...
   * @returns
   */
  async sign(value: string, tenantId: string, keyId?: string): Promise<string> {
    if (!keyId) {
      keyId = await this.getKid(tenantId);
    }
    return firstValueFrom(
