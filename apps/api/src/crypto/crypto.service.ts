...
export class CryptoService implements OnModuleInit {
      ...
      if (this.configService.get<boolean>('CONFIG_IMPORT')) {
        ...
        for (const tenant of tenantFolders) {
          ...
          for (const file of files) {
            const payload = JSON.parse(readFileSync(join(path, file), 'utf8'));
  ...
  async hasCerts(tenantId: string, id?: string) {
    ...
    const publicKey = await this.keyService.getPublicKey('pem', tenantId, id);
    ...
    if (!existsSync(dummyKey)) {
      ...
      if (!existsSync(dummyKey)) {
        throw new Error(`Public key exists but ${dummyKey} is missing.`);
      }
    }
  ...
  getSignJwtCallback(tenantId: string): SignJwtCallback {
    return async (signer, { header, payload }) => {
      ...
      const privateThumbprint = await calculateJwkThumbprint({
        jwk: (await this.keyService.getPublicKey('jwk', tenantId)) as Jwk,
        hashAlgorithm: HashAlgorithm.Sha256,
        hashCallback,
      });
  ...
  getJwks(tenantId: string) {
    return this.keyService.getPublicKey('jwk', tenantId) as Promise<EC_Public>;
  }
