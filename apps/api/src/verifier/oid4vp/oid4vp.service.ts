...
export class Oid4vpService {
  ...
  async createAuthorizationRequest(session: Session): Promise<string> {
    ...
    try {
      ...
      const dcql_query = JSON.parse(
        JSON.stringify(values.dcql_query).replace(/<PUBLIC_URL>/g, host)
      );

      if (this.registrarService.isEnabled()) {
        const registrationCert = JSON.parse(
          JSON.stringify(values.registrationCert).replace(/<PUBLIC_URL>/g, host)
        );
      ...
      this.sessionLogger.logAuthorizationRequest(logContext, {
        ...
        nonce,
        regCert,
        dcqlQueryCount: Array.isArray(dcql_query) ? dcql_query.length : 1,
      });
      ...
      const request = {
        payload: {
          ...
          client_metadata: {
            jwks: {
              keys: [
                this.encryptionService.getEncryptionPublicKey(session.tenantId),
              ],
            },
            vp_formats: {
              mso_mdoc: {
            ...
            client_name: this.configService.getOrThrow<string>('RP_NAME'),
  ...
  async createRequest(
    ...
    const params = {
      client_id: `x509_san_dns:${hostname}`,
      request_uri: `${this.configService.getOrThrow<string>(
        'PUBLIC_URL'
      )}/oid4vp/request/${values.session}`,
    };
  ...
  async getResponse(body: AuthorizationResponse, session: Session) {
    ...
    try {
      ...
      if (session.webhook) {
        await this.webhookService.sendWebhook(session, logContext, credentials);
      }
