 ...
 * @param rawEnv The raw environment variables.
 * @returns True if the key is effectively required, false otherwise.
 */
export function isEffectivelyRequired(
    key: string,
    rawEnv: Record<string, any>,
): boolean {
    const testEnv = { ...rawEnv };
    delete testEnv[key]; // pretend it's missing

    const { error } = VALIDATION_SCHEMA.validate(testEnv, {
        abortEarly: false,
        allowUnknown: true,
        convert: true,
    });
    ...
    // Look for "any.required" (and friends) on the specific key path
    return error.details.some(
        (d) =>
            Array.isArray(d.path) &&
            d.path.length === 1 &&
            d.path[0] === key &&
            (d.type === "any.required" || d.type.endsWith(".required")),
    );
}
